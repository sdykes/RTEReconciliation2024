...
SFPO241121 <- read_csv("SunFruitPackOuts241121.csv", show_col_types = F) |>
...
BinsPackedByBinDeliveryID <- DBI::dbGetQuery(con,
                                             "SELECT 
	                                                BinDeliveryID,
...
RunSummary <- SFRunReco |>
  ...
  select(-c(StorageTypeID,MaturityID,PickNoID))
...
TestTable <- RunReco |>
  left_join(MasterDataByPS, by = c("FarmCode","ProductionSite")) |>
